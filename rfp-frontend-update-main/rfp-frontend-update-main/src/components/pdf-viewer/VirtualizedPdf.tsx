...
const VirtualizedPDF = forwardRef<PdfFocusHandler, VirtualizedPDFProps>(
  ({ file, scale, setIndex, setScaleFit, setNumPages, type }, ref) => {
          ...
          await pdf
            ...
            .then(
              (page: {
                ...
                console.log(pageWidth,newWidthPx);
    ...
    // console.log("apiresponse",apiResponse[activeQuery]?.chunks[activeChunkIndex]);
    // console.log("apiresponse",apiResponse[activeQuery]?.chunks[activeChunkIndex]?.chunk_url);
    
    
    return (
      <div>
      {/* <>
        {console.log(apiResponse[activeQuery]?.chunks[activeChunkIndex]?.chunk_url)}
      </> */}
      <div
          ...
          <Document
            ...
            file={
              activeChunkIndex === -1
                ...
                : (
                    apiResponse &&
                    apiResponse[activeQuery]?.chunks &&
                    apiResponse[activeQuery]?.chunks[activeChunkIndex]?.chunk_url
                  ) as string // Assuming chunk_url is a string
            }
            // file={file.url}
