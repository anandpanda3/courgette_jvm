  ...
  useEffect(() => {
    const fetchDataSequentially = async () => {
      for (const [index, question] of queries.entries()) {
        try {
          const responseData = await backendClient.fetchQueryWithScore("/processquery/", question, 50);
          if (responseData) {
    ...
    <>
      <PdfFocusProvider>
        <SplitPane split="vertical" minSize={50} defaultSize="50%">
          <div className="flex h-[100vh] flex-col items-center border-r-2 bg-white">
            <div className="flex h-[44px] w-full items-center justify-between border-b-2">
              <div className="flex w-full items-center justify-between">
                <button
                  ...
                  className="ml-4 flex items-center justify-center rounded px-2 font-light text-[#494b52] hover:text-gray-900"
                >
